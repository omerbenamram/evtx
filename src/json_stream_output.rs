...
impl<W: Write> JsonWriter<W> {
    fn new(writer: W) -> Self {
        // Small buffer to coalesce tiny writes
        const JSON_WRITER_BUF_CAP: usize = 2048;
        Self {
            writer,
            buf: Vec::with_capacity(JSON_WRITER_BUF_CAP),
        }
    }
    ...
    fn write_bytes(&mut self, bytes: &[u8]) -> IoResult<()> {
        // If incoming chunk is larger than remaining capacity, flush first.
        if bytes.len() > self.buf.capacity() - self.buf.len() {
            self.flush()?;
        }
        // If the incoming chunk itself is bigger than the buffer capacity, bypass the buffer.
...
struct ObjectContext {
    ...
    suspended_attrs: Option<FastMap<String, SmallVec<[(String, FlatScalar); 4]>, ahash::RandomState>>,
}
...
impl<W: Write> JsonStreamOutput<W> {
    ...
    fn flush_all_suspended_into_object(&mut self, idx: usize) -> SerializationResult<()> {
        ...
        let mut attrs: FastMap<String, SmallVec<[(String, FlatScalar); 4]>, ahash::RandomState> = self.stack
            [idx]
            .suspended_attrs
            .take()
            .unwrap_or_else(|| FastMap::with_hasher(self.hasher.clone()));
