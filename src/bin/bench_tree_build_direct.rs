//! Loopable binary for profiling direct BinXML → IR parsing (no token iterator).

#[cfg(feature = "bench")]
mod imp {
    use bumpalo::Bump;
    use evtx::binxml::bench::{TreeBuildCache, build_tree_from_binxml_bytes_direct_in_bump};
    ...
    pub fn run() -> Result<(), Box<dyn std::error::Error>> {
        ...
        let bump: &'static mut Bump = Box::leak(Box::new(Bump::new()));
        for _ in 0..loops {
            chunk.arena.reset();
            bump.reset();
            ...
            let root =
                build_tree_from_binxml_bytes_direct_in_bump(binxml, &chunk, &mut cache, bump)?;
...
fn main() {
    eprintln!("`bench_tree_build_direct` requires `--features bench`");
}
